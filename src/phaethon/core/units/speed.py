...
from .. import vmath as _vmath
from .temperature import Kelvin
from .length import Meter
from .time import Second

@_axiom.bound(min_val=0, msg="Speed (scalar magnitude) cannot be negative!", abstract=True)
...
class KilometerPerSecond(SpeedUnit):
    symbol = "km/s"
    aliases = [
        "km/sec", "km / s", "kilometer per second", "kilometers per second",
...
class MilePerSecond(SpeedUnit):
    symbol = "mi/s"
    aliases = [
        "mi/sec", "mi / s", "mile per second", "miles per second",
        "mile/second", "miles/second"
    ]
...
class InchPerMinute(SpeedUnit):
    symbol = "in/min"
    aliases = [
        "ipm", "in / min", "inch per minute", "inches per minute",
        "inch/minute", "inches/minute"
    ]
    base_multiplier = _const.INCH_TO_METER / 60.0
...
@_axiom.derive(299792458 * (Meter / Second))
